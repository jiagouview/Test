...
<div>
    ...
    <nav-bread><span slot="bread">订单详情</span></nav-bread>
      ...
      <div class="checkout-order">
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <a class="btn btn--m" @click="prev">上一步</a>
          </div>
          <div class="next-btn-wrap">
            <button class="btn btn--m btn--red" @click="next">去付款</button>
          </div>
        </div>
      </div>
    </div>
...
import {getSelectGoods, deleteSelectGoods, getOrderNum} from 'api/orderList'
    ...
    methods: {
        next() {
            ...
            this.goodsList.forEach(function(item){
                that._deleteSelectGoods(item.productId)
            })
            getOrderNum(this.userId,this.finallyMoney).then((res) => {
                if(res.code == ERR_OK) {
                    that.$router.push({
                        path:'/orderSuccess/'+res.cartId+'/'+that.finallyMoney
                    })
                }
            })
        ...
        getGoodsList() {
            ...
            let that = this
            getSelectGoods(this.userId).then((res) => {
                if(res.code == ERR_OK) {
                    this.goodsList = res.data
                    this.goodsList.forEach(function(item) {
                        that.totalMoney += parseInt(item.products[0].price * item.count)
                    })
                    that.discount = that.totalMoney * 0.01
                    that.sax = that.totalMoney * 0.002
                    that.finallyMoney = that.totalMoney + that.discount + that.sax + that.freight
                    that.finallyMoney = Math.floor(that.finallyMoney)
                }else {
                    alert('出现错误')
                }
            })
        ...
        // 删除所有的商品
        _deleteSelectGoods(productId) {
            deleteSelectGoods(this.userId, productId).then((res) => {
                if(res.code == ERR_OK) {
